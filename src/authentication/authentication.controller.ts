...
export class AuthenticationController implements IController {
  ...
  private initializeRoutes() {
    this.router.get(`${this.path}/me`, authMiddleware, this.getMyProfile);
    this.router.post(
      `${this.path}/register`,
    ...
    this.router.get(
      `${this.path}/login-with-google`,
      GoogleAuth.authenticate(["profile", "email"])
    );
    this.router.get(
      `${this.path}/login-with-google/callback`,
      GoogleAuth.callbackAuth(),
    ...
    this.router.post(
      `${this.path}/refresh`,
      validationMiddleware(RefreshAccessTokenDto),
      this.refreshToken
    );
    this.router.post(
  ...
  private registration = async (
    ...
    try {
      ...
      response
        ...
        .send({
          user,
          message: "Ссылка для проверки отправлена на вашу электронную почту!",
        });
    } catch (error) {
      next(error);
    }
  ...
  private firstStepVerify = async (
    ...
    try {
      ...
      response.redirect(
        ...
        }/stepVerification?email=${encodeURIComponent(
          user.email
  ...
  private secondStepVerify = async (
    ...
    const {
      ...
      otpCode,
      rememberMe,
    }: { email: string; otpCode: string; rememberMe: boolean } = request.body;
    try {
  ...
  private loggingIn = async (
    ...
    try {
      ...
      response
        .status(status)
        .send({ accessToken, refreshToken, user, message });
    } catch (error) {
      next(error);
    }
  };

  private googleCallbackHandler = async (
    ...
    try {
      const { refreshToken, accessToken } =
        await this.authenticationService.googleCbHandler(request.user);

      response.redirect(
        `${process.env.APP_FRONT_URL}/success/with-google?accessToken=${accessToken}&refreshToken=${refreshToken}`
      );
    } catch (error) {
      next(error);
    }
  ...
  private refreshToken = async (
    ...
    try {
      const { refreshToken, rememberMe } = request.body as {
  ...
  private resetPassword = async (
    ...
    try {
      ...
      await this.authenticationService.resetUserPassword(resetPasswordData, userId);
